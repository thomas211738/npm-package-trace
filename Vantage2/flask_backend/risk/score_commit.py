...
def score_commit(diff: str):
    ...
    # suspicious postinstall/preinstall scripts that hit the network
    ...
    # child_process + network calls (download & execute)
    uses_child_process = (
        "child_process" in lower
        or "exec(" in lower
        or "spawn(" in lower
    )
    uses_network = (
    ...
    # stealing env vars/secrets/keys
