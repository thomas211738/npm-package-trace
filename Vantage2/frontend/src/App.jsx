            ...
            <div className="mt-6 bg-white p-4 rounded-lg shadow-md">
                ...
                {(() => {
                  ...
                  return (
                    <div className={`font-medium ${summaryColor}`}>
                      <p>
                        Highest Commit Risk: <strong>{highest}</strong>
                      </p>
                      <p>
                        Average Risk Score:{" "}
              ...
              {scanResults.repo && (
                <p className="text-gray-700 mb-2">
                  ...
                  {scanResults.repo.owner}/{scanResults.repo.repo}
                </p>
              )}
                      ...
                      <td className="p-2 space-x-1 space-y-1">
                        {c.flags?.length ? (
                          c.flags.map((f, i) => (
                            <span
                              key={i}
                              className={`inline-block px-2 py-1 rounded-full text-xs font-semibold ${getFlagColor(
                                f
                              )}`}
                            >
                              {f}
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
              <div className="bg-white p-6 rounded-lg shadow-xl max-w-4xl w-full max-h-[80vh] overflow-y-auto">
                <h2 className="text-xl font-bold mb-4">
                  Commit {activeCommit.sha.slice(0, 7)}
                </h2>
                <p className="text-gray-700 mb-2">
                  <strong>Message:</strong> {activeCommit.message}
                </p>
                <p className="text-gray-700 mb-2">
                  <strong>Flags:</strong>{" "}
                  {activeCommit.flags.length
                    ? activeCommit.flags.join(", ")
                    : "None"}
                </p>
                <h3 className="font-semibold mt-4 mb-2">Diff:</h3>
                <pre className="bg-gray-900 text-green-200 p-4 rounded-lg text-xs whitespace-pre-wrap overflow-x-auto">
{activeCommit.diff || "No diff available"}
                </pre>
                <button
