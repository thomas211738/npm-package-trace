        ...
        <div className="w-full max-w-7xl px-4">
          ...
          <div className="relative mb-10">
            <div className="flex">
              ...
              <button
                ...
                className="bg-blue-600 text-white px-5 py-3 rounded-r-md hover:bg-blue-700 focus:ring-2 focus:ring-blue-500"
              >
                {scanLoading ? (
                ...
                {isLoading ? (
                  ...
                  results.map((result) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-600 truncate">{result.package.description || "No description available"}</div>
                    </div>
                  ))
                ) : (
          ...
          {selectedPackage && (
            <div className="bg-white p-4 rounded-md shadow-md">
              ...
              {selectedPackage.description && <p className="mt-2"><strong>Description:</strong> {selectedPackage.description}</p>}
            </div>
          )}
          {scanError && <div className="mt-4 text-red-600 text-center">Error: {scanError}</div>}
          {scanResults && (
...
{activeCommit.diff}
                ...
                <button onClick={() => setActiveCommit(null)} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded">Close</button>
              </div>
            </div>
          )}
          <footer className="mt-10 text-gray-500 text-sm text-center">Built by Group 1 — Boston University EC521 (2025)</footer>
        </div>
      </div>
    </div>
  );
